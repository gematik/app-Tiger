...
class TestHostHeaderForwardCallback {

  private TigerProxy tigerProxy;
  private HostHeaderForwardCallback callback;
  ...
  void setUp() {
    ...
    callback = new HostHeaderForwardCallback(tigerProxy);
  }

  @Test
  void matches_withValidHostHeader_shouldReturnTrue() {
    var request = HttpRequest.request().withHeader("Host", "example.com:9090");
    assertThat(callback.matches(request)).isTrue();
  }

  @Test
  void matches_withEmptyHostHeader_shouldReturnFalse() {
    var request = HttpRequest.request().withHeader("Host", "");
    assertThat(callback.matches(request)).isFalse();
  }

  @Test
  void matches_withNoHostHeader_shouldReturnFalse() {
