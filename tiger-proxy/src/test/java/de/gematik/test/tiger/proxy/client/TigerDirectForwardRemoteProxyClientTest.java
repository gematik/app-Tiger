    ...
    void forwardMessagesToNonExistingServer_shouldReceiveException() throws Exception {
        executeRemoteProxyTestWithMessagesAndVerification(
            ...
            (requestCalls, responseCalls, serverCalled) -> {
                ...
                assertThat(tigerRemoteProxyClient.get().getRbelMessages().getFirst().getRawContent())
                    .containsExactly(request);
                assertThat(tigerRemoteProxyClient.get().getReceivedRemoteExceptions())
                    .hasSize(1);
            },
            port -> TigerGlobalConfiguration.readString("free.port.200"));
    }

    private void executeRemoteProxyTestWithMessagesAndVerification(ThrowingConsumer<Socket> clientActionCallback,
        ThrowingConsumer<Socket> serverAcceptedConnectionCallback, VerifyInteractionsConsumer verifyInteractionsConsumer,
        Function<Integer, String> serverPortMapper) throws Exception {
        ...
        try {
            executeTestRun(
                ...
                serverPort -> {
                    tigerProxyApplication.set(new SpringApplicationBuilder()
                        .bannerMode(Mode.OFF)
                        .properties(Map.of("tigerProxy.directReverseProxy.hostname", "127.0.0.1",
                            "tigerProxy.directReverseProxy.port", serverPortMapper.apply(serverPort),
                            "server.port", TigerGlobalConfiguration.readString("free.port.100")))
                        .sources(TigerProxyApplication.class)
                        .web(WebApplicationType.SERVLET)
                        .initializers()
                        .run());
                    final TigerProxy tigerProxy = tigerProxyApplication.get().getBean("tigerProxy", TigerProxy.class);
                    tigerRemoteProxyClient.set(new TigerRemoteProxyClient("http://localhost:" + tigerProxy.getAdminPort(),
