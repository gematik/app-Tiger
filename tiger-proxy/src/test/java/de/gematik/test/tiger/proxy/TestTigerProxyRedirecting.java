...
class TestTigerProxyRedirecting extends AbstractFastTigerProxyTest {
    ...
    void rewriteOfLocationHeaderShouldNotRemoveAdditionalHeaders() {
        //We need a new UnirestInstance to be able to change its own configuration without affecting the other unit tests
        ...
        try (UnirestInstance unirestNoRedirects = Unirest.spawnInstance()) {
            ...
            HttpResponse<String> response = unirestNoRedirects.get("http://localhost:" + tigerProxy.getProxyPort()
                    + "/withAdditionalHeaders").asString();

            assertThat(response.getStatus()).isEqualTo(302);//see AbstractTigerProxyTest for expected value.
