...
public class ZionRequestMatchDefinition {
   ...
   @JsonProperty //JsonProperty is here to still be able to serialize the field
    ...
    public PathMatchingResult matchPathVariables(RbelElement requestRbelMessage, TigerJexlContext context) {
        ...
        String fullUrlFromRequest = requestRbelMessage.getFacet(RbelHttpRequestFacet.class)
                .map(f -> f.getPath().getRawStringContent()).orElse("");
        // If there is no remaining path yet, we take it from the request, otherwise we load what is remaining from the context
        String pathFromRequest = (String) context.getOptional( TigerJexlContext.REMAINING_PATH_FROM_REQUEST)
                .orElseGet( () -> getPathFromFullUrl(fullUrlFromRequest));

        PathPattern pathPattern = PathPatternParser.defaultInstance.parse(path);

        PathPattern.PathRemainingMatchInfo matchInfo = pathPattern.matchStartOfPath(PathContainer.parsePath(pathFromRequest));
