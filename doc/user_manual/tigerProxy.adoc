...
which supports both http- and https-traffic (so you do not have to differentiate between the two). Additionally, they have an admin-port (configurable via `tigerProxy.adminPort`). This provides a webui to monitor the traffic,
...
The default CA used by the Tiger Proxy can be found here: https://github.com/gematik/app-Tiger/blob/master/tiger-proxy/src/main/resources/CertificateAuthorityCertificate.pem
