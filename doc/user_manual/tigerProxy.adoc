...
The Tiger Proxy is a proxy server that can be deployed in three distinct modes:

* **Local Tiger Proxy** — automatically started by the Test Environment Manager (TestenvMgr) as part of every Tiger test suite.
It is the central traffic recording point for the test suite.

* **Remote Tiger Proxy** (`type: tigerProxy` in the `servers` block) — an additional proxy instance started and managed by the TestenvMgr alongside other servers.
It is useful to intercept traffic between two components that do not communicate through the local test suite proxy.

* **Standalone Tiger Proxy** — a self-contained Spring Boot JAR or Docker image that runs independently of any test suite.
It is the recommended choice for permanently running traffic recorders, sidecar containers, or mesh-setup nodes.
See xref:tigerProxy.adoc#_standalone_proxy[Tiger Standalone Proxy] for details.

==== Configuration structure by deployment mode

The configuration structure differs depending on the deployment mode.

*Local Tiger Proxy* and *Standalone Tiger Proxy* share the same flat structure — all properties are placed directly under the `tigerProxy` key:
...
*Remote Tiger Proxy* (declared as a server of `type: tigerProxy`) places its proxy-specific properties under `tigerProxyConfiguration` inside the server entry:
...
All three deployment modes expose the same two ports: a **proxy port** for HTTP/HTTPS traffic, and an **admin port** providing the Tiger Proxy Web UI (described in detail xref:tigerWebUI.adoc#_web_ui[here]), a REST interface to manage routes and modifications at runtime, and a WebSocket/STOMP interface to stream RBel messages to other Tiger Proxy instances.

The following example shows a complete Tiger Proxy configuration with all available properties.
When used as a *local* or *standalone* proxy the properties are placed directly under `tigerProxy` (without an additional `tigerProxyConfiguration` nesting).
When used as a *remote* proxy server they are placed under `servers.<name>.tigerProxyConfiguration`.
  ...
  myTigerProxy:
    ...
    tigerProxyConfiguration:
      ...
      directReverseProxy:
        ...
        tcpIdleTimeoutInSeconds: 5.0        # OPTIONAL TCP keep-alive timeout in seconds (default: 5), null = disabled
...
This is required if the target server expects the proxy to authenticate itself using a client certificate.
You can provide the identity as a string (e.g., myIdentity.p12;changeit;JKS) or as a map with filename, password, and storeType.
...
==== directReverseProxy TCP Idle Timeout (Keep-Alive)

The `directReverseProxy` configuration supports an optional `tcpIdleTimeoutInSeconds` parameter to enable the configuration of the TCP keep-alive behavior for connections to the backend server.
This is useful for long-lived connections (like SMTP) that may have long periods of inactivity to prevent the proxy or server from closing the connection:
...
tigerProxy:
    tigerProxyConfiguration:
        directReverseProxy:
            ...
            port: 3858
            ignoreConnectionErrors: false
            tcpIdleTimeoutInSeconds: 5.0
----

When configured, the proxy sends periodic keep-alive pings to the backend server at the specified interval (in seconds).
This helps maintain connection stability in scenarios with long periods of inactivity, such as SMTP communication where the client may wait a long time before sending the next command.

**Important**: The keep-alive interval should be **shorter** than the socket read timeout (typically 20 seconds).
For example, if you set `tcpIdleTimeoutInSeconds` to 5 seconds, the keep-alive pings every 5 seconds will reset the socket timeout counter before it expires, keeping the connection alive indefinitely.

How it works:
1. Keep-alive pings are sent at the configured interval on idle connections 2. Each ping resets the socket read timeout counter 3. As long as pings arrive before the timeout expires, the connection stays open 4. When the client sends actual data, the ping cycle resumes after the next idle period
...
RBelPath is a powerful expression language specifically designed for navigating and analyzing messages within the Tiger Proxy.
Inspired by well-known concepts like XPath and JSONPath, RBelPath is tailored to the unique requirements of HTTP traffic and complex testing scenarios.
With RBelPath, testers and developers can precisely access any part of a message, extract values, filter, or compare data—regardless of whether the content is JSON, XML, JWT, HTTP headers, or other supported formats.
This flexibility greatly simplifies the validation and evaluation of test data, making message analysis more efficient and adaptable.
...
(The first example executes the RbelPath and returns a list of all matching elements; the second one returns an Optional containing a single result.
...
The Tiger Standalone Proxy repackages the Tiger Proxy as a self-contained, runnable Spring Boot JAR and as a Docker image.
It is the recommended way to run a Tiger Proxy **outside of a Tiger test suite** — for example as a permanently running traffic recorder, as part of a mesh setup, or as a sidecar container.

==== Standalone Proxy vs. Remote Tiger Proxy (`type: tigerProxy`)

Both the Standalone Proxy and a remote proxy server (`type: tigerProxy` in the `servers` block) use the same underlying Tiger Proxy engine with identical capabilities (routes, TLS, modifications, RBel parsing, mesh setup, etc.).
The key difference lies in how and where they are managed:

[%autowidth,cols="1,2,2"]
|===
| | `type: tigerProxy` (remote server) | Tiger Standalone Proxy

| **Started by**
| TestenvMgr (automatically on test suite start)
| Manually (JAR or Docker)

| **Lifecycle**
| Tied to the test suite — started and stopped with it
| Independent — runs as a long-lived process or container

| **Configuration location**
| `servers.<name>.tigerProxyConfiguration` in `tiger.yaml`
| Directly under `tigerProxy` in `application.yaml`

| **`dependsUpon` / server ordering**
| Supported — can depend on other servers
| Not applicable

| **Typical use case**
| Sniffer between two components in the same test environment
| Permanent traffic recorder, mesh node, sidecar container
|===

Use `type: tigerProxy` when the additional proxy is part of a test environment that is set up and torn down with each test run.
Use the Standalone Proxy when the proxy should run independently of any particular test suite — for example, as a constantly running traffic forwarder in a mesh setup, or in CI/CD pipelines where the proxy is shared across multiple test runs.

==== Configuration

All configuration options described in xref:tigerProxy.adoc#_tiger_proxy[the Tiger Proxy chapter] apply equally to the Standalone Proxy.
The properties are placed directly under the `tigerProxy` key — the same flat structure used by the local Tiger Proxy.

Place the file as `application.yaml` next to the JAR (Spring Boot picks it up automatically), or supply it explicitly via `--spring.config.additional-location`.

The following example shows a typical `application.yaml` for a standalone instance acting as a forward proxy with a mesh connection to a remote Tiger Proxy:

.Example `application.yaml` for the Tiger Standalone Proxy
[source,yaml]
----
tigerProxy:
  # ---- Ports ---------------------------------------------------
  adminPort: 9090          # Web UI and REST API reachable at http://localhost:8080
  proxyPort: 8080          # HTTP/HTTPS forward- and reverse-proxy port

  # ---- Display name --------------------------------------------
  name: "my-standalone-proxy"

  # ---- Offline mode --------------------------------------------
  # Set to true to serve the Web UI from the classpath (no internet required).
  localResources: false

  # ---- Routes --------------------------------------------------
  proxyRoutes:
    - from: http://backend.example.de/   # forward-proxy route
      to: http://internal.service:8080/

  # ---- TLS -----------------------------------------------------
  tls:
    serverRootCa: "ca-cert.pem;ca-key.pem;PKCS8"   # custom CA for on-the-fly cert generation

  # ---- Mesh setup ----------------------------------------------
  # Connect to another Tiger Proxy and receive its traffic.
  trafficEndpoints:
    - http://other-tiger-proxy:8080
  downloadInitialTrafficFromEndpoints: true

  # ---- Traffic file --------------------------------------------
  fileSaveInfo:
    writeToFile: true
    filename: "traffic.tgr"
  ...
  # ---- Buffer sizes --------------------------------------------
  rbelBufferSizeInMb: 512

# ---- Spring Boot server port (= the admin port above) ---------
# This must match tigerProxy.adminPort. Required for health checks.
server:
  port: 9090
----

NOTE: The `server.port` Spring Boot property must always match `tigerProxy.adminPort`.
The container health check and the mesh STOMP endpoint both use this port.
If you change `adminPort`, update `server.port` accordingly.

There is one property exclusive to the Standalone Proxy (not available for `type: tigerProxy` servers):

[source,yaml]
----
tigerProxy:
  localResources: false   # true = serve Web UI assets from classpath (offline/air-gapped environments)
----

==== Running tiger proxy as a standalone JAR
...
Download the JAR and start it with:

[source,bash]
----
java -jar tiger-standalone-proxy-<version>.jar
----

To supply a custom configuration, place an `application.yaml` next to the JAR — Spring Boot picks it up automatically.
Alternatively, point Spring Boot at any file via `--spring.config.additional-location`:

[source,bash]
----
java -jar tiger-standalone-proxy-<version>.jar \
     --spring.config.additional-location=file:./my-config.yaml
----

All properties described in xref:tigerProxy.adoc#_tiger_proxy_basics[Tiger Proxy basics] can be used directly in that file.

==== Running tiger proxy as a Docker container

You can also run the Tiger Proxy as a Docker container:
...
IMPORTANT: Always keep the admin port mapped to 8080 inside the container.
The container health check connects to port 8080 — if you change this mapping, the health check will fail and the container will be considered unhealthy.

To supply a custom configuration, mount an `application.yaml` into the container:
...
Once the Standalone Proxy is running, the following endpoints are available on the admin port:

- **Tiger Proxy Web UI** — traffic viewer and live filter:
`http://localhost:{adminPort}/webui`
- **Swagger UI** — interactive browser-based API explorer:
`http://localhost:{adminPort}/swagger-ui/index.html`
- **OpenAPI JSON spec**:
`http://localhost:{adminPort}/v3/api-docs`
- **OpenAPI YAML spec**:
`http://localhost:{adminPort}/v3/api-docs.yaml`

The OpenAPI spec covers all Tiger Proxy REST endpoints, including route management (`/route`), traffic modifications (`/modification`), tracing points (`/tracingpoints`), traffic push (`/traffic`), and the Web UI endpoints.
