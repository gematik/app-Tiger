...
Tiger can parse OTLP payloads sent via gRPC over HTTP/2 when `activateRbelParsing` is enabled.
Supported content types are:

* `application/grpc`
* `application/grpc+proto`
* `application/grpc+json` (best-effort, payload must still be a valid OTLP JSON payload)

Supported OTLP export RPC paths:
...
* Compressed gRPC payloads are detected but not decompressed.
* OTLP parsing only runs for the listed Export RPCs.
