...
There is, however, one additional property specific to the standalone proxy:

[source,yaml]
----
# Load all resources (JS, CSS) from the classpath instead of via CDN.
# Set to true if the environment has no internet access.
...
The spec covers all Tiger-Proxy REST endpoints, including route management (`/route`), traffic modifications (`/modification`), tracing points (`/tracingpoints`), traffic push (`/traffic`), and the Web UI endpoints.
