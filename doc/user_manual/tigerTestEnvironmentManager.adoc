...
The helm chart node allows to start a helm chart from the configured image. The helm chart is started and the
server is ready when the pod is running and the pod is ready, if port-forward is used (if exposedPorts are set),
then port-forwarding is also done and the startup is finished and the service can be used for testing.
...
servers:
  helmChart_Demis_HLS:
    ...
    source:
      - demis-gematik/hospital-location-service
    ...
    helmChartOptions:
      ...
      # if empty no port forwarding is done.
      # The syntax is <POD_NAME_OR_REGEX>,<LOCAL_PORT>:<FORWARDING_PORT>[,<LOCAL_PORT>:<FORWARDING_PORT>]*
      exposedPorts:
        - hospital-location-service.*,8080:8090
        - someOther-service.*,8080:8096,10000:8096
----
