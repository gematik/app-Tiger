...
The file needs to be located in the same folder as the `tiger.yaml` file. Additionaly, you can configure in the tiger
configuration a default profile which will be loaded when none is set via environment variable or system property. See xref:tigerTestLibrary.adoc#_tiger_test_lib_configuration[here] for more details.
