...
* *Input field*  … allows you to filter the incoming messages with a RbelPath or a JEXL expression by using the "Set filter" button. You can also search for text or a regex in this input field, but then you need to place the search text inside quotation marks (").
* *DropUp* … allows you to filter the incoming messages for requests from or to a certain server. The JEXL expression will then be automatically filled in the input field and the user triggers the filter by pressing the "Set filter" button.
* *Reset* … allows you to delete all recorded messages so far. This will delete all messages on the Tiger Proxy!
* *Save* … allows you to save the current HTML page or the raw traffic as tgr file.
