...
POST form data requests contain the data as Url encoded query string in the body of the request. There is no easy way to decode this data generically within Rbel/JEXL. To help you ease the situation for POST we do have a helper JEXL method: ```!{rbel:urlEncoded('value')}```
