...
----
The steps four, five and six allow to pause the validation test suite. Please note, these steps are only modified for the Workflow UI and don't work on a regular console (no failure, there is just no pause).
...
* **@PRIO:N** .... Mandatory - priority number (1-4)
...
** @Afo:AAAAA ... a link to a defined requirement (Anforderung) in Polarion, where AAAAA matches the custom field "cfAfoId"
** @AF-ID:AAAAA ... a link to a defined requirement (Anwendungsfall) in Polarion, where AAAAA matches the custom field "cfAfId"
...
For more details on how to perform the synchronisation and how to also upload generated test run reports to Polarion and Aurora, please check the README.md in the PolarionToolbox project on the Gematik GitLab.
