...
in the response to the doc string beneath the step applying the JSONChecker comparison algorithm.

The JSONChecker class performs a multilevel comparison of JSON structures, checking for identical existence of keys and matching values.

Values will be first checked for containing "${json-unit.ignore}" in which case the value comparison will default to true, then for equals and finally for regex matching.

JSON object attributes starting with four underscores "____" are optional and allow the doc string
to contain attributes to be checked for value ONLY if it exists in the test JSON RBelPath node.

JSON Arrays are compared in lenient mode (strict order not necessary).
...
This class supports reading YAML and JSON configuration files and mapping them to POJOs based upon Jackson and SnakeYaml.
It also allows overwriting configured values via Java system properties and environment variables like in Spring Boot.
For more details please check the code and its usages in the Tiger test framework.

==== TestContext / TestVariables

In BDD/Cucumber you will often find it difficult to share test data / state of the test run between steps or scnearios.
These classes provide support in that they create and maintain static thread aware Hashmaps to store and retrieve data.
For more details please check the code and its usages in the Tiger test framework.

==== TokenSubstituteHelper

Eases the task of replacing tokens with "configurable" patterns in a string with values from a map.
For more details please check the code and its usages in the Tiger test framework.
...
// TODO TGR-312 JULIAN shall we keep this here or to be moved to the Tiger Proxy heading?
// === Using Tiger Proxy in standalone mode to monitor traffic
