...
CAUTION: Currently, multithreaded or parallel test runs are not supported.
...
lib:
    ...
    # Flag whether to generate intermediate Serenity HTML reports
    # after each feature file completes. This allows you to view
    # test progress while the suite is still running.
    # The report is written to target/site/serenity/index.html
    # and a machine-readable progress.json is placed alongside it.
    # Deactivated by default.
    createIntermediateReports: false
...
=== Intermediate Serenity Reports

By default, Serenity HTML reports are only generated at the end of a test suite run.
If you want to monitor test progress while the suite is still running you can enable intermediate report generation.
When activated, the Serenity HTML report is regenerated after each feature file completes and written to `target/site/serenity/index.html`.

A machine-readable `progress.json` file containing status information (completed features, pass/fail counts, timing estimates) is placed alongside it.

To enable this feature, add the following to your `tiger.yaml`:
...
[#_validation_message_sort_order]
==== Validation iteration order
...
WARNING: `TIMESTAMP` changes how every validator step pairs requests/responses; existing suites may need review.
...
[#_complete_set_of_steps_in_validation_glue_code]
...
RBelObjects are internal data structures used by the Tiger test framework to represent and manipulate parsed message content, such as JSON, XML, or token formats. They allow you to access, modify, and assert values at specific paths within a message using RBelPath expressions (see xref:tigerProxy.adoc#_rbel_path_details[Understanding RBelPath]). RBelObjects enable flexible validation, transformation, and serialization of message data during automated tests.
...
image::screenshots/bannerInLog.jpg[title="Banner in log",width=800]

Supports ANSI coloring and a set of different fonts.
Furthermore, all banner messages are displayed and highlighted in the Workflow UI, as seen in xref:tigerWorkflowUI.adoc#_banner_in_workflowui[this] figure.
