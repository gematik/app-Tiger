...
For full details how to specify a JSON Schema, refer to the external resource: https://json-schema.org/understanding-json-schema/index.html[Understanding JSON Schema].
...
The step "wait for user abort" allows to pause the validation test suite and is mainly used in demo scenarios allowing the manual tester to perform demo transactions that will be logged and saved to HTML reports but are not validated.
This step subsequently terminates the test execution.
