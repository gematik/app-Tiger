...
public class RbelHtmlRenderingToolkit {
    ...
    public ContainerTag convert(final RbelElement element, final Optional<String> key) {
        if (element.getRawContent() != null
            ...
            return addNotes(element, span(RbelHtmlRenderer.buildOversizeReplacementString(element)).withClass("is-size-7"));
        }
        ...
        return convertUnforced(element, key)
            .orElseGet(() -> {
                if (element.hasFacet(RbelBinaryFacet.class)) {
                    ...
                    return addNotes(element, span(performElementToTextConversion(element)).withClass("is-size-7"));
                }
            });
    }
    ...
    public DomContent menuTab(final RbelElement rbelElement) {
        ...
        if (rbelElement.hasFacet(RbelRequestFacet.class)) {
            return div().withClass("ms-1").with(
                a().with(
                    div(span(getElementSequenceNumber(rbelElement)).withClass("tag is-info is-light me-1"),
                    ...
                    div(rbelElement.getFacetOrFail(RbelRequestFacet.class).getMenuInfoString())
                        ...
                        .withClass("is-size-6 ms-3")
                ).withHref("#" + uuid).withClass("mt-3 is-block")
            );
        } else {
            return a(span(getElementSequenceNumber(rbelElement)).withClass("tag is-info is-light me-1"),
                i().withClass("fas fa-reply"),
                text(" RESPONSE"),
                div(rbelElement.getFacet(RbelResponseFacet.class).map(RbelResponseFacet::getMenuInfoString).orElse(""))
                    .attr("style", "text-overflow: ellipsis;overflow: hidden;")
                    .withClass("is-size-6 ms-3"))
                .withHref("#" + uuid).withClass("menu-label ms-1 mt-3 is-block has-text-success");
        }
    }
                ...
                body()
                    .with(
                        ...
                        section().withClass("main-content").with(
                            section().withClass("row is-vcentered header").with(
                                div().withClass("col is-one-fifth is-inline-block logo").with(
                                ...
                                div().withClass("is-inline-block").with(
                                    ...
                                    .withClass("col is-size-6 is-italic is-clearfix")
                            )
                        ),
                        section().withClass("row is-fullheight").with(
                            ...
                            div().withClass("col ms-6").with(
                                div("Created " +
                                    ...
                                    .withClass("created is-italic is-size-6 is-pulled-right me-6"),
                                div().with(
                                    elements.stream()
    ...
    public List<DomContent> packAsInfoLine(String parameterName, DomContent... contentObject) {
        return List.of(div().withClass("row is-multiline is-mobile").with(
            div().withClass("col-sm-4")
