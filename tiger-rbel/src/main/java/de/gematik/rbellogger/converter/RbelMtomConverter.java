...
public class RbelMtomConverter implements RbelConverterPlugin {
    ...
    private class RbelMtomConverterExecutor {
        ...
        public void execute() {
            final Optional<MediaType> vauContentType = getVauContentType(parentNode)
                .or(() -> getMtomContentType(parentNode));
            ...
            rootPart = mtomParts.stream()
                .filter(mtomPart -> mtomPart.getMessageHeader()
                    .getOrDefault("Content-ID", "")
                    .equals(vauContentType.get().parameters().get("start").stream().findAny().orElse("")))
                .findAny()
        ...
        private Optional<MediaType> extractContentType(RbelElement rbelElement) {
            return rbelElement.getFacet(RbelHttpHeaderFacet.class)
                .stream()
