...
public class RbelJwtWriter implements RbelElementWriter {
  ...
  public byte[] write(
    ...
    return createUpdatedJws(oldTargetModifiedChild, new String(newContent, UTF_8), jwtFacet)
        .getBytes(UTF_8);
  }

  private String createUpdatedJws(
      RbelElement oldTargetModifiedChild, String newContent, RbelJwtFacet jwtFacet) {
    ...
    try {
      ...
      if (jwtFacet.getSignature() == oldTargetModifiedChild
          && !newContent.startsWith(
              new String(RbelJwtSignatureWriter.VERIFIED_USING_MARKER, UTF_8))) {
        return compactSerialization.substring(0, compactSerialization.lastIndexOf('.'))
            + "."
            + newContent;
      }
  ...
  private Key extractJwsKey(RbelJwtFacet jwtFacet) {
    return jwtFacet
        .getSignature()
        .getFacet(RbelJwtSignature.class)
  ...
  private List<Map.Entry<String, String>> extractJwtHeaderClaims(
      RbelElement oldTargetModifiedChild, String newContent, RbelJwtFacet jwtFacet) {
  ...
  private String extractJwsBodyClaims(
      RbelElement oldTargetModifiedChild, String newContent, RbelJwtFacet jwtFacet) {
  ...
  private Optional<Key> getKeyBasedOnEncryptionType(RbelKey rbelKey) {
    if (rbelKey.getKey().getAlgorithm().equals("HS256")
        || rbelKey.getKey().getAlgorithm().equals("HS512")
        || rbelKey.getKey().getAlgorithm().equals("HS384")) {
      return Optional.ofNullable(rbelKey.getKey());
    } else {
      return rbelKeyManager.findCorrespondingPrivateKey(rbelKey.getKeyName()).map(RbelKey::getKey);
    }
  }
