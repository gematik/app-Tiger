...
public class RbelVauErpWriter implements RbelElementWriter {
  ...
  public boolean canWrite(RbelElement oldTargetElement) {
    return oldTargetElement.hasFacet(RbelVauErpFacet.class)
        && oldTargetElement
            .getFacet(RbelVauErpFacet.class)
  ...
  public byte[] write(
    ...
    final Optional<RbelKey> decryptionKey =
        oldTargetElement.getFacet(RbelVauErpFacet.class).flatMap(RbelVauErpFacet::getKeyUsed);
  ...
  private byte[] rewriteErpVauResponse(
      RbelElement oldTargetElement, byte[] newContent, RbelKey decryptionKey) {
    String[] pParts =
        oldTargetElement
            .getFacet(RbelVauErpFacet.class)
    ...
    return encrypt(
        ArrayUtils.addAll(
            (pParts[0] + " " + pParts[1] + " ").getBytes(StandardCharsets.UTF_8), newContent),
        decryptionKey.getKey().getEncoded());
  }

  @SneakyThrows()
  private byte[] rewriteErpVauRequest(
      RbelElement oldTargetElement, byte[] newContent, RbelKey decryptionKey) {
    String[] pParts =
        oldTargetElement
            .getFacet(RbelVauErpFacet.class)
    ...
    ECPublicKey otherSidePublicKey =
        extractPublicKeyFromVauMessage(oldTargetElement.getRawContent());
    ...
    byte[] newCiphertext =
        encrypt(
            ArrayUtils.addAll(
                (pParts[0] + " " + pParts[1] + " " + pParts[2] + " " + pParts[3] + " ")
