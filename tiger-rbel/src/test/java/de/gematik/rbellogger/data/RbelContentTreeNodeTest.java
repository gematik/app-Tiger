...
class RbelContentTreeNodeTest {
    ...
    void getParentNodeTest() {
        ...
        assertEquals("blub", msg1.findElement("$.blub.foo").orElseThrow().getParentNode().getKey().orElseThrow());
    }
    ...
    void getChildNodesWithKeyTest() {
        assertTrue(msg1.getChildNodesWithKey().stream().map(Map.Entry::getKey).toList().contains("blub"));
    }
    ...
    void findRootElementTest() {
        assertEquals(2, msg2.findElement("$.blub.foo.some").orElseThrow().findRootElement().getChildNodesWithKey().size());
        assertTrue(msg2.findElement("$.blub.foo.some").orElseThrow().findRootElement().getChildNodesWithKey().stream().allMatch(k ->  (k.getKey().equals("blub") || k.getKey().equals("blab"))));
    }
    ...
    void findKeyInParentElementTest() {
        ...
        assertEquals("foo", msg2.findElement("$.blub.foo").orElseThrow().findKeyInParentElement().orElseThrow());
    }
