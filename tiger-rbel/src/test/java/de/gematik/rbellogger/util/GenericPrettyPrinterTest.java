...
class GenericPrettyPrinterTest {
  ...
  private final GenericPrettyPrinter<ASN1Encodable> genericPrettyPrinter =
      new GenericPrettyPrinter<>(
          isLeaf, Object::toString, asn1 -> Streams.stream((Iterable<ASN1Encodable>) asn1));

  @Test
  void printAsn1P12() throws IOException {
    ASN1Encodable rootNode =
        readData(FileUtils.readFileToByteArray(new File("src/test/resources/vau_cert.der")));
    final String prettyPrint = genericPrettyPrinter.prettyPrint(rootNode);
    System.out.println(prettyPrint);
    assertThat(prettyPrint).isNotBlank();
  }
