...
class RbelUriConverterTest {
  ...
  void convertAbsolutePathWithQueryParameters() {
    ...
    assertThat(rbelElement.getFacet(RbelUriFacet.class)).isPresent();
    assertThat(rbelElement.getFacetOrFail(RbelUriFacet.class).getBasicPathString())
        .isEqualTo("/foobar");
    assertThat(
            rbelElement.getFacetOrFail(RbelUriFacet.class).getQueryParameters().stream()
  ...
  void domainAndProtocolAndQuery_shouldConvertNested() {
    ...
    final RbelElement code =
        rbelElement.getFacetOrFail(RbelUriFacet.class).getQueryParameter("code").get();
    assertThat(code.hasFacet(RbelJweFacet.class)).isTrue();
    assertThat(rbelElement.traverseAndReturnNestedMembers()).contains(code);
  }
  ...
  void urlEscapedParameterValues_shouldContainOriginalContent() {
    final RbelElement rbelElement =
        RbelLogger.build()
            .getRbelConverter()
            .convertElement("/endpoint?scope=pairing%20openid&foo=bar+schmar", null);
  ...
  void simpleUrlWithoutQuery() {
    ...
    assertThat(rbelElement.hasFacet(RbelUriFacet.class)).isTrue();
    ...
    assertThat(rbelElement.getFacetOrFail(RbelUriFacet.class).getQueryParameters()).isEmpty();
  }

  @Test
  void longSpecialCaseParameter() {
    final String sourceParameter = RandomStringUtils.insecure().nextPrint(3000);
    ...
    final RbelElement rbelElement = RbelLogger.build().getRbelConverter().convertElement(url, null);
