...
class RbelSicctConverterTest {
  ...
  void shouldRecognizeSicctMessages() throws IOException {
    FileUtils.writeStringToFile(
        new File("target/sicctFlow.html"), RbelHtmlRenderer.render(rbelLogger.getMessages()));
  ...
  void testForBasicAttributesInSicctEnvelope() {
    ...
    assertThat(history.getFirst().findElement("$.messageType").get().seekValue())
        .contains(SicctMessageType.C_COMMAND);
    assertThat(history.getFirst().findElement("$.srcOrDesAddress").get().getRawContent())
        .isEqualTo(new byte[] {0, 0});
    assertThat(history.getFirst().findElement("$.sequenceNumber").get().getRawContent())
        .isEqualTo(new byte[] {1, 0x41});
    assertThat(history.getFirst().findElement("$.abRfu").get().getRawContent())
        .isEqualTo(new byte[] {0});
    assertThat(history.getFirst().findElement("$.length").get().getRawContent())
