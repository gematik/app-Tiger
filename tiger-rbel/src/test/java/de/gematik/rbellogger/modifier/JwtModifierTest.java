...
class JwtModifierTest extends AbstractModifierTest {
  ...
  void modifyJwtHeader_shouldContainModifiedContent() throws IOException {
    final RbelElement message =
        readAndConvertCurlMessage("src/test/resources/sampleMessages/jwtMessage.curl");
  ...
  void jwtSignature_changeVerifiedUsingToPuk() throws IOException {
    ...
    assertThat(message.findElement("$..verifiedUsing").get())
        .extracting(el -> el.seekValue(String.class).get())
        .isNotNull()
        .isNotEqualTo("puk_idpEnc");
    assertThat(modifyMessageAndParseResponse(message).findElement("$..verifiedUsing"))
        .get()
        .extracting(el -> el.seekValue(String.class).get())
  ...
  void jwtSignature_changeVerifiedUsingToPrk() throws IOException {
    ...
    assertThat(modifyMessageAndParseResponse(message).findElement("$..verifiedUsing").get())
        .extracting(el -> el.seekValue(String.class).get())
        .isEqualTo("puk_idpEnc");
  }
            ...
            RbelModificationDescription.builder()
                ...
                .replaceWith(
                    Base64.getUrlEncoder()
                        .encodeToString(
                            "hullabulla, but not a signature".getBytes(StandardCharsets.UTF_8)))
                .build());

    assertThat(modifyMessageAndParseResponse(message).findElement("$.body.signature").get())
  ...
  void modifyJwt_correctSecret() throws IOException {
    RbelKey secretKey =
        RbelKey.builder()
            ...
            .key(
                new SecretKeySpec(
                    ("n2r5u8x/A?D(G-KaPdSgVkYp3s6v9y$B").getBytes(StandardCharsets.UTF_8),
                    AlgorithmIdentifiers.HMAC_SHA256))
            .build();
    rbelLogger.getRbelKeyManager().addKey(secretKey);

    final RbelElement message =
        readAndConvertCurlMessage("src/test/resources/sampleMessages/jwtMessageWithSecret.curl");
