...
describe("testing StepUpdate class", () => {
  ...
  test("empty JSON full rbelMetaData applies correctly", () => {
    const step = StepUpdate.fromJson(
      JSON.parse(
        '{ "rbelMetaData":[{"uuid": "someUUID", "menuInfoString": "GET /some/path", "recipient": "someRecipient", "sender": "someSender", "sequenceNumber": 1}]}',
      ),
    );
  ...
  test("full JSON applies correctly", () => {
    const step = StepUpdate.fromJson(
      JSON.parse(
        '{"description":"Then User requests classes with parameter xyz=4","tooltip":"tooltip","status":"PASSED","stepIndex":3,"rbelMetaData":[]}',
      ),
    );
    expect(step.description).toBe(
  ...
  test("complete merge works correctly", () => {
    const step1 = StepUpdate.fromJson(
      JSON.parse(
        '{ "description": "desc1", "tooltip": "tooltip1", "status": "FAILED", "stepIndex":3, "rbelMetaData":[]}',
      ),
    );
    const step2 = StepUpdate.fromJson(
      JSON.parse(
        '{ "description": "desc2", "tooltip": "tooltip2", "status": "PENDING", "stepIndex":4, "rbelMetaData":[{"uuid": "someUUID", "menuInfoString": "GET /some/path", "recipient": "someRecipient", "sender": "someSender", "sequenceNumber": 1}]}',
      ),
    );
    step1.merge(step2);
  ...
  test("empty description merge works correctly", () => {
    const step1 = StepUpdate.fromJson(
      JSON.parse('{ "description": "desc1", "status": "FAILED" }'),
    );
    const step2 = StepUpdate.fromJson(
      JSON.parse(
        '{ "status": "PENDING","stepIndex":4, "rbelMetaData":[{"uuid": "someUUID", "menuInfoString": "GET /some/path", "recipient": "someRecipient", "sender": "someSender", "sequenceNumber": 1}]}',
      ),
    );
    step1.merge(step2);
  ...
  test("empty description/tooltip merge works correctly", () => {
    const step1 = StepUpdate.fromJson(
      JSON.parse(
        '{ "description": "desc1", "tooltip" : "tooltip1", "status": "FAILED" }',
      ),
    );
