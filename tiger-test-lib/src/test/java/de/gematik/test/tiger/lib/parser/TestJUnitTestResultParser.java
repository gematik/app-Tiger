...
class TestJUnitTestResultParser {
    ...
    void testJunitResultParseOK() {
        ...
        parser.parseDirectoryForResults(results, Paths.get("src", "test", "resources", "testdata", "parser", "junit").toFile());

        assertThat(results.keySet()).hasSize(357);
        assertThat(
            results.get("de.gematik.idp.tests.aforeport.TestAfoJUnitTestResultParser:testJunitResultParseOK").getStatus())
