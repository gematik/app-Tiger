...
public class TigerProxyGlue {
     ...
     * The parameter uses the TigerPkiIdentity-syntax used also for the YAML-configuration. For more information refer to the
     * user manual, section "Configuring PKI identities in Tiger Proxy’s tls section".
     * Be aware: This method reboots the internal mockserver, leading to a short period in which the local TigerProxy can not forward traffic.
     * (It will still function in a mesh-setup, no traffic will be lost). Before the method returns the mockserver is successfully restarted.
     *
     * @param certificateFile The certificate to use. Use TigerPkiIdentity-syntax (e.g. "my/file/name.p12;p12password")
     */
    ...
    public void setLocalTigerProxyForwardMutualTlsIdentity(final String certificateFile) {
        changeTlsSettingForLocalTigerProxy(certificateFile, (cfg, crt) -> cfg.getTls().setForwardMutualTlsIdentity(crt));
    }
    ...
    public void setLocalTigerProxyServerIdentity(final String certificateFile) {
        changeTlsSettingForLocalTigerProxy(certificateFile, (cfg, crt) -> cfg.getTls().setServerIdentity(crt));
    }
    ...
    public void setLocalTigerProxyRootCa(final String certificateFile) {
        changeTlsSettingForLocalTigerProxy(certificateFile, (cfg, crt) -> cfg.getTls().setServerRootCa(crt));
    }
    ...
    private static void changeTlsSettingForLocalTigerProxy(String certificateFile, BiConsumer<TigerProxyConfiguration, TigerConfigurationPkiIdentity> configurationChanger) {
        final TigerProxy localTigerProxy = TigerDirector.getTigerTestEnvMgr().getLocalTigerProxyOptional()
            .orElseThrow(() -> new TigerProxyGlueException("Could not change settings for the local TigerProxy: The local TigerProxy is inactive"));
        final TigerConfigurationPkiIdentity newIdentity = new TigerConfigurationPkiIdentity(certificateFile);
