...
class XDynamicSidebarTests extends AbstractTests {
    ...
    void testSidebarIsClosedWhenClickedOnDoubleArrow() {
        ...
        assertAll(
            ...
            () -> assertThat(page.locator("#test-sidebar-statusbox .test-sidebar-status-features").isVisible()).isFalse(),
            () -> assertThat(page.locator("#test-sidebar-statusbox .test-sidebar-status-scenarios").isVisible()).isFalse(),
    ...
    void testFeatureBoxClickOnLastSecnario() {
        ...
        assertAll(
            () -> assertThat(featureTitle.trim()).startsWith(sidebarTitle)
        );
    }
    ...
    void testFindFailedStepInFeatureBoxAndInExecutionPane() {
        page.querySelector("#test-tiger-logo").click();
        String sidebarTitle = page.locator("#sidebar-left .test-failed").first().locator("..").textContent();
        String featureTitle = page.locator(".test-execution-pane-scenario-title .test-failed").first().locator("..").textContent().trim();
        ...
        assertThat(page.locator(".test-step-status-skipped").first().locator("..").textContent())
            .isEqualTo("And TGR assert \"!{rbel:currentRequestAsString('$.path')}\" matches \"\\/not_a_file\\/?\"");
    }
