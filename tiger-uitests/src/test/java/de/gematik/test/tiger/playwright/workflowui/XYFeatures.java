...
class XYFeatures extends AbstractTests {
    ...
    void testFeaturesAndScenarioAmount() {
        ...
        assertAll(
            ...
            () -> assertThat(page.locator("#test-sidebar-featurelistbox .test-sidebar-feature-name").count()).isEqualTo(2),
    ...
    void testFeaturesNames() {
        page.querySelector("#test-tiger-logo").click();
        String featureName = page.locator("#test-sidebar-featurelistbox .test-sidebar-feature-name").first().textContent();
        String featureName2 = page.locator("#test-sidebar-featurelistbox .test-sidebar-feature-name").last().textContent();
    ...
    void testScenarioNames(String scenarioName, int counter, int index) {
        page.querySelector("#test-tiger-logo").click();
        assertThat(page.locator("#test-sidebar-featurelistbox .test-sidebar-scenario-name").nth(counter).textContent().trim()).contains(scenarioName);
        if (index > 0) {
            assertThat(page.locator("#test-sidebar-featurelistbox .test-sidebar-scenario-name").nth(counter).locator(".test-sidebar-scenario-index").textContent()).isEqualTo("[" + index + "]");
        }
    }
    ...
    void testXDataTablesExsistFirstRow(String testName, int counter) {
        ...
        assertThat(page.locator(".table-data-table").nth(4).locator("tr").first().locator("td").nth(counter).textContent()).isEqualTo(testName);
    }
    @ParameterizedTest()
    @CsvSource(delimiter = '|', textBlock = """      
        client_id                   | 0  |
    ...
    void testXDataTablesExsistSecondRow(String testName, int counter) {
        page.querySelector("#test-execution-pane-tab").click();
        assertThat(page.locator(".table-data-table").nth(4).locator("tr").last().locator("td").nth(counter).textContent()).isEqualTo(testName);
    }
    ...
    void testScrollingToKlickedLastTestfile() {
        ...
        await()
            ...
            .untilAsserted(() ->
                assertThat(((Double)page.evaluate("document.getElementsByClassName('test-execution-pane-scenario-title')[24].getBoundingClientRect().bottom")) < 800).isTrue());

        Double bottom = (Double) page.evaluate("document.getElementsByClassName('test-execution-pane-scenario-title')[24].getBoundingClientRect().bottom");
        Double top = (Double) page.evaluate("document.getElementsByClassName('test-execution-pane-scenario-title')[24].getBoundingClientRect().top");
    ...
    void testScrollingToKlickedFirstTestfile() {
        page.querySelector("#test-tiger-logo").click();
        page.locator(".test-sidebar-scenario-name").first().locator(".scenarioLink").click();
        Double bottom = (Double)page.evaluate("document.getElementsByClassName('test-execution-pane-scenario-title')[0].getBoundingClientRect().bottom");
        Double top = (Double)page.evaluate("document.getElementsByClassName('test-execution-pane-scenario-title')[0].getBoundingClientRect().top");
        Integer clientHeight = (Integer) page.evaluate("document.documentElement.clientHeight");
        Integer innerHeight = (Integer) page.evaluate("window.innerHeight");
        var viewHeight = Math.max(clientHeight, innerHeight);
        assertTrue( (!(bottom < 0 || top - viewHeight >= 0)), "Scenario is not visible in viewport");
    ...
    void testRequestsAreCorrectInScenario() {
        page.querySelector("#test-execution-pane-tab").click();
        int count = page.locator(".test-execution-pane-scenario-title").first().locator("..").locator(".rbelmessage").count();
        assertThat(count).isEqualTo(2);
        assertThat(page.locator(".test-execution-pane-scenario-title").first().locator("..").locator(".rbelmessage").nth(0).locator(".test-rbel-link").textContent()).isEqualTo("1");
        assertThat(page.locator(".test-execution-pane-scenario-title").first().locator("..").locator(".rbelmessage").nth(1).locator(".test-rbel-link").textContent()).isEqualTo("3");
    }
}
