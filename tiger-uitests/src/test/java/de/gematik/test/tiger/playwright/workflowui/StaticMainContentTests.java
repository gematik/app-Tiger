...
class StaticMainContentTests extends AbstractTests {
    ...
    void testServerLogPaneButtonsExists() {
        ...
        assertAll(
            ...
            () -> assertThat(page.locator("#test-server-log-pane-server-remoteTigerProxy").isVisible()).isTrue(),
...
//    }
    ...
    void testServerPanServerButtonsExist(String server) {
        page.querySelector("#test-server-log-tab").click();
        assertThat(
            page.locator("#test-server-log-pane-buttons").locator(".test-server-log-pane-server-" + server)
                .isVisible()).isFalse();
    }
}