...
public class TigerConfigurationLoader {
  ...
  public synchronized void addCustomMapper(Consumer<MapperBuilder> customizer) {
  ...
  private <T> Optional<T> instantiateConfigurationBean(
    ...
    try {
      return Optional.of(objectMapper.convertValue(targetTree, configurationBeanClass));
    } catch (Exception e) {
  ...
  public <T> T instantiateConfigurationBean(
    ...
    return deserializeType(
        (final TreeNode node) -> objectMapper.convertValue(node, configurationBeanType),
  ...
  private boolean isArray(ObjectNode value) {
    if (value.propertyNames().stream().anyMatch(s -> !NumberUtils.isParsable(s))) {
      return false;
    }
    try {
      final List<Long> keys =
          value.propertyNames().stream().mapToLong(Long::parseLong).sorted().boxed().toList();
      int i = 0;
      for (Long key : keys) {
        if (key != i++) {
          return false;
  ...
  private String homogenizeKeysInMapAndReturnCorrectedKey(
    ...
    for (String toBeReplacedKey : position.propertyNames()) {
  ...
  public Map<String, String> readMap(TigerConfigurationKey reference) {
    return retrieveMap(reference).entrySet().stream()
        .filter(entry -> entry.getKey().isBelow(reference))
        .collect(
            Collectors.toMap(
  ...
  public List<String> readList(TigerConfigurationKey reference) {
    return retrieveMap(reference).entrySet().stream()
        .filter(entry -> entry.getKey().isBelow(reference))
        .map(Entry::getValue)
        .toList();
  ...
  public void putValue(TigerConfigurationKey key, Object value) {
    putValue(key, value, ConfigurationValuePrecedence.RUNTIME_EXPORT);
  }
