                ...
                stage('Setup Test environment') {
                    steps {
                        withCredentials([string(credentialsId: 'GITHUB.API.Token', variable: 'GITHUB_TOKEN')]) {
                            sh """
                                cd tiger-maven-plugin
                                rm -f mvn-testenv-log.txt
                                mvn -U -P setup-testenv -Dtiger.testenv.cfgfile=src/test/resources/de/gematik/test/tiger/maven/adapter/mojos/testExternalLocalJar.yaml | tee mvn-testenv-log.txt
                               """
                        }
                    }
                }
                stage('Run connection test') {
                    steps {
                        sh """
                            cd tiger-maven-plugin
                            mvn -P run-connection-test verify
                        """
                        // clean up mvn-testenv-log.txt and shutdown testenv as soon as tests have ended to minimize time container is running
                        sh """
                          cd tiger-maven-plugin
                          rm -f mvn-testenv-log.txt
