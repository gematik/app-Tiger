...
/**
 * Integration test for the maven plugin to start up a test environment.
 * Test setup is that in tiger-integration-tests/tiger-maven-plugin-integration-tests.Jenkinsfile we do run
 * the maven plugin once with setup-testenv that starts a winstone server on free.port.0 and logs the output to log.txt
 * In parallel we do run this test as a second maven job, check for log.txt and try to retrieve the health check url
 * of the started winstone server. Finally, we check that we do get a status 200 back and all is fine.
 */
@Slf4j
public class EnvironmentMojoIT {
    ...
    public static void parseWinstoneUrl() {
        ...
        await().pollInterval(500, TimeUnit.MILLISECONDS).atMost(30, TimeUnit.SECONDS).until(() -> {
            if (f.exists()) {
